...
let stringToNumbers = function(array){
    //take array of stringed numbers and make them one big string of numbers
    let numberArray = array.join('');
    //take string of numbers and turn them into an array of numbers
    numberArray = numberArray.split('').map(Number);
    return numberArray;
}
//save function to a variable
const nums = stringToNumbers(strNums);

// Find the sum of the even values
const sumEvens = function(array) {
    //Get all the evens of the array and create new array of the even numbers
    let evenArray = array.filter(function(element){
    return (element % 2 === 0)
    })
    //Create variable to keep the sum of the even numbers 
    let sum = 0;
    //loop through even number array
    for(let i = 0; i < evenArray.length ; i++){
        //add even number to sum of even numbers
        sum = sum + evenArray[i];
    }
    //return the variable that returns the sum of the even numbers
    return sum;
};
console.log(sumEvens(nums)) ;

// Find the index of the first "Austin" value (the value plus its index equals 512)
let findAtxIdx = function(array){
    //loop through array
    for(let i = 0; i < array.length; i++){
        let index = i;
        //if the index of the array and the value of the index equals 512 return that index
        if(index + array[i] == 512){
            return index;
        }
    }
}
//save function to variable
const atxIdx = findAtxIdx(nums);
...
const weatherStates = weather.map(function(weather){
    return weather.weather_state_name
    })
    .filter(function(weather,index,array){
        return array.indexOf(weather) === index
    })
